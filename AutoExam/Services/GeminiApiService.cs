...
/// <summary>
/// Lo que devolvio un lote. Lleva el aviso de truncado ademas de las preguntas porque el
/// bucle necesita distinguir dos cosas que se veian iguales: un lote que rindio poco porque
/// el material daba poco, y uno que rindio poco porque la respuesta no entro en el cupo de
/// salida. El primero no tiene arreglo; el segundo se arregla pidiendo lotes mas chicos.
/// </summary>
internal readonly record struct LoteGenerado(List<Pregunta> Preguntas, bool Truncado);
...
public class GeminiApiService
{
    ...
    /// <summary>
    /// Piso al que puede bajar el tamanio de lote cuando la respuesta se corta. Tres
    /// preguntas entran en cualquier cupo de salida; por debajo, el prompt y el material
    /// pesarian mas que el resultado y cada peticion rendiria casi nada.
    /// </summary>
    private const int MinPreguntasPorLote = 3;
    ...
    /// </summary>
    private const int MaxLotesDeRelleno = 10;

    /// <summary>
    /// Tope absoluto de peticiones de generacion por examen, relleno incluido.
    ///
    /// Antes eran 6, elegidas para no quemar la cuota diaria. Resulto ser el limite
    /// equivocado: cambiaba un examen incompleto por una cuota intacta, y un examen
    /// incompleto no sirve para estudiar. La decision aca es al reves —se prefiere tardar
    /// varios minutos y gastar mas cuota antes que entregar 5 preguntas de 10—, con un
    /// tope igual para que un pedido imposible no se lleve el dia entero.
    /// </summary>
    private const int MaxPeticionesPorExamen = 16;

    /// <summary>
    /// Lotes seguidos sin preguntas nuevas antes de rendirse. Tres y no dos: con lotes que
    /// se van achicando, un intento fallido ya no significa que el modelo no vaya a cumplir,
    /// significa que todavia no encontro el tamanio que le entra.
    /// </summary>
    private const int MaxLotesEsteriles = 3;
    ...
    /// <summary>
    /// Si el modelo de la sesion deja apagar el razonamiento. Se asume que si y se corrige
    /// al primer rechazo, en vez de mantener una lista de que modelo lo permite: esa lista
    /// envejece con cada generacion que saca Google, y el modelo ya nos contesta la pregunta.
    /// </summary>
    private static bool _razonamientoApagable = true;

    /// <summary>
    /// Vuelve al estado inicial lo que el servicio aprende en caliente: el techo de salida y
    /// si el modelo deja apagar el razonamiento. Existe para las pruebas, porque son campos
    /// estaticos: sin poder reiniciarlos, un caso contamina al siguiente y la sonda deja de
    /// medir lo que dice medir.
    /// </summary>
    public static void ReiniciarAprendizajeDeSesion()
    {
        _topeTokensVigente = int.MaxValue;
        _razonamientoApagable = true;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        for (int lote = 0; lote < presupuesto; lote++)
        {
            ...
            // lote da el mismo corte una y otra vez, asi que se achica a la mitad y el
            // examen se completa en mas viajes pero completo. Es el mecanismo que hace que
            // "tarda mas" sea la respuesta correcta a "no entra", en vez de "salen menos".
            if (generadas.Truncado && porLote > MinPreguntasPorLote)
            {
                int antes = porLote;
                porLote = Math.Max(MinPreguntasPorLote, porLote / 2);

                progreso?.Report(
                    $"La respuesta se corto por tamanio: se bajan los lotes de {antes} a {porLote} preguntas.");
                diagnostico.Registrar($"Lote {lote + 1}: respuesta cortada; lotes reducidos de {antes} a {porLote}.");
            }

            int nuevas = 0;
            int repetidas = 0;

            foreach (var p in generadas.Preguntas)
            {
        ...
        // Un examen corto no lanza excepcion, asi que hasta ahora no dejaba rastro en ningun
        // lado: el diagnostico moria en memoria y el log solo tenia los fallos totales. Sin
        // esto, "me genero 5 de 10" es imposible de investigar despues de que paso.
        if (preguntas.Count < solicitud.CantidadPreguntas)
        {
            RutasApp.RegistrarError(
                $"Examen incompleto: {preguntas.Count} de {solicitud.CantidadPreguntas} preguntas " +
                $"(modelo {solicitud.Modelo}, lotes de {porLote})",
                new InvalidOperationException(diagnostico.Resumen()));
        }
    ...
    /// </summary>
    private async Task<LoteGenerado> GenerarLoteConReintentosAsync(
        SolicitudGeneracion solicitud,
        AnilloDeClaves claves,
        ...
        CancellationToken ct)
    {
        ...
        bool truncadoEnAlgunIntento = false;
        ...
        for (int i = 0; i < intentos.Count; i++)
        {
            ...
            try
            {
                ...
                if (generadas.Preguntas.Count > 0)
                {
                    if (i > 0)
                    {
                        progreso?.Report(
                            $"Lote {numeroLote}: recuperado con \"{nombre}\" ({generadas.Preguntas.Count} preguntas).");
                    }

                    return generadas;
                }

                // Sin preguntas pero con la respuesta cortada, el aviso de truncado sigue
                // sirviendo aunque este intento no haya aportado nada: el lote era grande.
                if (generadas.Truncado)
                {
                    truncadoEnAlgunIntento = true;
                }
        ...
        return new LoteGenerado(new List<Pregunta>(), truncadoEnAlgunIntento);
    }
    ...
    private async Task<LoteGenerado> GenerarLoteAsync(
        SolicitudGeneracion solicitud,
        AnilloDeClaves claves,
        ArchivoRemoto? pdfRemoto,
        List<FragmentoTexto> fragmentos,
        ...
        CancellationToken ct)
    {
        ...
        // El razonamiento interno del modelo se cobra del MISMO cupo de salida que las
        // preguntas, y cuanto piensa cambia en cada pedido. Por eso el espacio que sobraba
        // para el JSON variaba solo, y un examen de 10 volvia con 3, 5 u 8 preguntas segun
        // el humor del modelo. Apagarlo devuelve el cupo entero al examen y hace que la
        // cantidad deje de ser una loteria. La prueba de conexion ya lo apagaba; la
        // generacion, que es donde importa, no.
        if (_razonamientoApagable)
        {
            cuerpo["generationConfig"]!["thinkingConfig"] = new JsonObject { ["thinkingBudget"] = 0 };
        }
        ...
        catch (GeminiException ex) when (_razonamientoApagable && EsRechazoDelRazonamiento(ex.Message))
        {
            // Hay modelos que no dejan apagarlo. Se anota para toda la sesion y se sigue sin
            // el campo: los lotes adaptativos alcanzan igual para completar el examen.
            _razonamientoApagable = false;
            cuerpo["generationConfig"]!.AsObject().Remove("thinkingConfig");

            diagnostico.Registrar(
                $"El modelo \"{solicitud.Modelo}\" no permite apagar el razonamiento; se sigue sin eso.");
        ...
        return new LoteGenerado(mapeadas, truncado);
    }
    ...
    /// <summary>
    /// Un 400 que se queja del campo de razonamiento. Se mira el nombre del campo y no solo
    /// el codigo: un 400 puede venir por cualquier otra cosa del cuerpo, y reintentar sin
    /// <c>thinkingConfig</c> en esos casos solo gastaria una peticion mas para el mismo error.
    /// </summary>
    private static bool EsRechazoDelRazonamiento(string mensaje)
        => mensaje.Contains("(400)", StringComparison.Ordinal)
           && (mensaje.Contains("thinking", StringComparison.OrdinalIgnoreCase)
               || mensaje.Contains("thought", StringComparison.OrdinalIgnoreCase));
        ...
        out bool truncado)
    {
        ...
        try
        {
            ...
            {
                // MAX_TOKENS no es un detalle para el log: es la senial de que el lote no
                // entra, y el bucle la usa para achicar los siguientes en vez de repetir
                // el mismo pedido demasiado grande hasta quedarse sin presupuesto.
                truncado = razon.Equals("MAX_TOKENS", StringComparison.OrdinalIgnoreCase);
    ...
    private static string ExplicarFinishReason(string razon) => razon.ToUpperInvariant() switch
    {
        "MAX_TOKENS" =>
            ...
            "La app ya achica los lotes sola cuando pasa esto; si se repite, el material " +
            "del alcance es demasiado denso para el modelo elegido.",
        "SAFETY" =>
